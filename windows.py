...
def search_get():
    address = var_ip.get()
    # 发送get报文，提取html
    request_data, head, var_html = utils.request_get(address)
    re_cdata = re.compile('//<!\[CDATA\[[^>]*//\]\]>', re.I)
    re_script = re.compile('<\s*script[^>]*>[^<]*<\s*/\s*script\s*>', re.I)  # Script
    re_style = re.compile('<\s*style[^>]*>[^<]*<\s*/\s*style\s*>', re.I)  # style
    var_html = re_cdata.sub('', var_html)
    var_html = re_script.sub('', var_html)  # 去掉SCRIPT
    var_html = re_style.sub('', var_html)  # 去掉style
    # 解析html
    my = htmlUtils.MyParser()
    my.feed(var_html)
    e = Example(win1, my.lableList).place(x=60, y=420)
...
def search_head():
    address = var_ip.get()
    var_html = ""
    request_data, recv_data = utils.request_head(address)
    comment_text.insert(INSERT, 'request:\r\n' + request_data)
    comment_text.insert(INSERT, 'response:\r\n' + recv_data + '\r\n' + '-' * 20 + '\r\n')
...
if __name__ == '__main__':
    ...
    button1=Button(win1, text="Connect Test",command=search).place(x=500 ,y=20)
    Label(win1, text="交互过程：", font=('Arial', 14)).place(x=30, y=80)
    comment_text = scrolledtext.ScrolledText(win1, width=65, height=15, undo=True,autoseparators=False, font=('Arial', 12))
